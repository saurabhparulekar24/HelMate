...
volatile char mqtt_msg_temp[64] = "1";
...
static void MQTT_HandleLockMessages(void);
static void MQTT_HandleIRMessages(void);
static void MQTT_HandleVersionMessages(void);
/******************************************************************************
 * Callback Functions
 ******************************************************************************/
...
void SubscribeHandlerLedTopic(MessageData *msgData)
{
    ...
    //     LogMessage(LOG_DEBUG_LVL, "\r\nRGB %d %d %d\r\n", rgb[0], rgb[1], rgb[2]);
    // }
        // Log the topic name for debugging purposes
    LogMessage(LOG_DEBUG_LVL, "\r\n %.*s", msgData->topicName->lenstring.len, msgData->topicName->lenstring.data);

    // Cast the payload to a string for easier comparison
    char* payload = (char*) msgData->message->payload;

    // Compare the payload with predefined commands and set the bike mode state accordingly
    if (strncmp(payload, "TURN_LEFT", strlen("TURN_LEFT")) == 0) {
        set_bike_mode_state_LED(TURN_LEFT);
    } else if (strncmp(payload, "TURN_RIGHT", strlen("TURN_RIGHT")) == 0) {
        set_bike_mode_state_LED(TURN_RIGHT);
    } else if (strncmp(payload, "SOLID_RED", strlen("SOLID_RED")) == 0) {
...
void SubscribeHandlerSystemStateTopic(MessageData *msgData){
    ...
    char* payload = (char*) msgData->message->payload;
    // Compare the payload with predefined commands and set the bike mode state accordingly
    if (strncmp(payload, "MODE_BIKE", strlen("MODE_BIKE")) == 0) {
        set_system_state_LED(MODE_BIKE);
    } else if (strncmp(payload, "MODE_LOCK", strlen("MODE_LOCK")) == 0) {
        set_system_state_LED(MODE_LOCK);
    } else if (strncmp(payload, "MODE_FUN", strlen("MODE_FUN")) == 0) {
        set_system_state_LED(MODE_FUN);
    }
}

void SubscribeHandlerFWTopic(MessageData *msgData){
    SerialConsoleWriteString("Downloading New Firmware");
...
void SubscribeHandlerResetTopic(MessageData *msgData){
    SerialConsoleWriteString("Reseting Hardware");
    system_reset();
    ...
    switch (type) {
        ...
        case MQTT_CALLBACK_CONNECTED:
            if (data->connected.result == MQTT_CONN_RESULT_ACCEPT) {
                ...
                mqtt_subscribe(module_inst, SYSTEM_STATE_TOPIC, 2, SubscribeHandlerSystemStateTopic);
                mqtt_subscribe(module_inst, FW_STATE_TOPIC, 2, SubscribeHandlerFWTopic);
                mqtt_subscribe(module_inst, RESET_TOPIC, 2, SubscribeHandlerResetTopic);
...
static void HTTP_DownloadFileTransaction(void)
{
    ...
    // Write Flag
    char test_file_name[] = "0:Application_New.txt";
    ...
    if (res != FR_OK) {
        ...
        SerialConsoleWriteString("Application_New.txt added!\r\n");
    }
...
static void MQTT_HandleTransactions(void)
{
    ...
    MQTT_HandleLockMessages();
    MQTT_HandleIRMessages();
    MQTT_HandleVersionMessages();
...
static void MQTT_HandleVersionMessages(void){
    snprintf(mqtt_msg, 63, "%s", FIRMWARE_VERSION);
    mqtt_publish(&mqtt_inst, VERSION_TOPIC, mqtt_msg, strlen(mqtt_msg), 1, 0);
}

static void MQTT_HandleIRMessages(void){
    struct SensorData ir_data;
    read_sensor(&ir_data);
    snprintf(mqtt_msg, 63, "{\'LEFT\':%d, \'RIGHT\':%d}", ir_data.left_side, ir_data.right_side);
    mqtt_publish(&mqtt_inst, IR_STATE_TOPIC, mqtt_msg, strlen(mqtt_msg), 1, 0);

}

static void MQTT_HandleLockMessages(void){
    enum lock_states current_lock_status = get_lock_state();
    snprintf(mqtt_msg, 63, "ALERT:%d", current_lock_status);
    mqtt_publish(&mqtt_inst, LOCK_STATE_TOPIC, mqtt_msg, strlen(mqtt_msg), 1, 0);
}
...
static void MQTT_HandleImuMessages(void)
{
    ...
    if (pdPASS == xQueueReceive(xQueueImuBuffer, &imuDataVar, 0)) {
        snprintf(mqtt_msg, 63, "{\'x\':%d, \'y\': %d, \'z\': %d}", imuDataVar.xmg, imuDataVar.ymg, imuDataVar.zmg);
...
void vWifiTask(void *pvParameters)
{
    ...
    if (xQueueWifiState == NULL || xQueueImuBuffer == NULL) {
